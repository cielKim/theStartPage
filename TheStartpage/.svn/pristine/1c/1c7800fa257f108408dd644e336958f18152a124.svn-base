...
# 알라딘 서점 컴퓨터 공학 Best 10
html = urlopen("http://www.aladin.co.kr/shop/common/wbest.aspx?BestType=Bestseller&BranchType=1&CID=2719")
...
print("알라딘 컴퓨터 공학 Best10 : "+str(indexAladin)+" 권")

# 교보문고 컴퓨터 공학 Best 10
html = urlopen("http://www.kyobobook.co.kr/categoryRenewal/categoryMain.laf?linkClass=3301&mallGb=KOR&orderClick=sgx")
bsObj = BeautifulSoup(html.read().decode('euc-kr','ignore'), "html.parser")
...
print("교보문고 컴퓨터 공학 Best10 : "+str(indexKyobo)+" 권")

# Yes24 컴퓨터 공학 Best 10
html = urlopen("http://www.yes24.com/24/category/bestseller?CategoryNumber=001001003031&sumgb=08")
bsObj = BeautifulSoup(html.read().decode('euc-kr','ignore'), "html.parser")
...
print("Yes24 컴퓨터 공학 Best10 : "+str(indexYes24)+"권")

comEngineertingTotal = set(aladin+kyobo+yes24)
print("< 3사 통합한 컴퓨터 공학 서적 수 :"+str(len(comEngineertingTotal))+"권 >")
...
sql = """delete from itbookbest where category = '컴퓨터공학'"""
cursor.execute(sql)

for bookIsbn13 in comEngineertingTotal:
    ...
    if(dd.split("|")[1].strip().endswith("역")): # 역자가 있는 경우 index 3번째가 출판일
